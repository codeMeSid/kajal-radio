...
export type DomeImage =
  | string
  | { src: string; alt?: string; href?: string; kicker?: string; title?: string };

type ImageRecord = { src: string; alt: string; href?: string; kicker?: string; title?: string };

type Item = ImageRecord & { x: number; y: number; sizeX: number; sizeY: number };
...
function buildItems(pool: DomeImage[], seg: number): Item[] {
  ...
  const normalizedImages: ImageRecord[] = pool.map((image) => {
...
function TileCaption({ kicker, title, alt }: Pick<ImageRecord, "kicker" | "title" | "alt">) {
  if (kicker || title) {
    return (
      <span className="item__caption">
        {kicker ? <span className="item__kicker">{kicker}</span> : null}
        {title ? <span className="item__title">{title}</span> : null}
      </span>
    );
  }
  if (!alt) return null;
  return <span className="item__caption">{alt}</span>;
}
...
function clientPoint(event: Event) {
...
export default function DomeGallery({
  ...
  useGesture(
    {
      onDragStart: ({ event }) => {
        ...
        stopInertia();
        const evt = clientPoint(event);
        if (!evt) return;
        draggingRef.current = true;
        movedRef.current = false;
        startRotRef.current = { ...rotationRef.current };
        startPosRef.current = evt;
      },
      onDrag: ({ event, last, velocity = [0, 0], direction = [0, 0], movement }) => {
        if (focusedElRef.current || !draggingRef.current || !startPosRef.current) return;
        const evt = clientPoint(event);
        if (!evt) return;
        const dxTotal = evt.x - startPosRef.current.x;
        const dyTotal = evt.y - startPosRef.current.y;
        if (!movedRef.current) {
  ...
  const activateTile = useCallback(
    (el: HTMLElement) => {
      ...
      if (openingRef.current) return;
      const href = el.parentElement?.dataset.href;
      if (href) {
        router.push(href);
        return;
      }
    ...
    [openItemFromElement, router],
  );

  const onHrefClick = useCallback((e: MouseEvent<HTMLAnchorElement>) => {
    if (draggingRef.current || movedRef.current || performance.now() - lastDragEndAt.current < 80) {
      e.preventDefault();
    }
  }, []);
              ...
              >
                {it.href ? (
                  <a
                    ...
                    href={it.href}
                    aria-label={it.alt || "Open page"}
                    onClick={onHrefClick}
                  >
                    <img src={it.src} draggable={false} alt="" />
                    <TileCaption kicker={it.kicker} title={it.title} alt={it.alt} />
                  </a>
                ) : (
                  <div
                    className="item__image"
